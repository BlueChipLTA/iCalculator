...
public class CalculatorResult extends AlignedTextView implements MenuItem.OnMenuItemClickListener,
    ...
    private static final String KEY_PREF_THEME = "_theme";
    ...
    public CalculatorResult(Context context, AttributeSet attrs) {
        ...
        SharedPreferences sharedPreferences = context.getSharedPreferences("calculator", Context.MODE_PRIVATE);
        int theme = sharedPreferences.getInt(KEY_PREF_THEME, 1);
        if (theme == THEME_MODE_DARK) {
            ...
            mExponentColorSpan = new ForegroundColorSpan(
                    ContextCompat.getColor(context, R.color.light_display_result_exponent_text_color));
        }
    ...
    private String getSelectionText() {
        ...
        String selection;
        if (start == end) {
            if (start == -1 && getText() != null && getText().toString().length() > 0) {
                selection = getText().toString().substring(0, getText().length());
            } else {
                selection = "";
            }
        } else {
    ...
    private void initPositions(int initPrecOffset, int msdIndex, int lsdOffset,
        ...
        if (lsdOffset < MAX_RIGHT_SCROLL) {
            ...
            // only display trailling zeroes when result is not fit to layout
            if (mMaxCharOffset < -1 && (mMaxCharOffset > -(MAX_TRAILING_ZEROES + 2) || mWholePartFits)) {
                mMaxCharOffset = -1;
            }
