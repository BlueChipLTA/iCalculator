...
package com.android.calculator2.expression;
...
import com.android.calculator2.db.ExpressionDB;
...
public class Evaluator implements CalculatorExpr.ExprResolver {
    ...
    private static final String KEY_PREF_MEMORY_SAVED = "memory_saved";
    ...
    Evaluator(Context context) {
        ...
        mMainExpr.mDegreeMode = mSharedPrefs.getBoolean(KEY_PREF_DEGREE_MODE, true);
    ...
    private static class InitialResult {
        ...
        InitialResult(UnifiedReal v, String s, int p, int idp) {
            errorResourceId = CalculatorActivity.INVALID_RES_ID;
            val = v;
            newResultString = s;
        ...
        boolean isError() {
            return errorResourceId != CalculatorActivity.INVALID_RES_ID;
        }
    }
        ...
        private void handleTimeout() {
            ...
            if (running && cancel(true)) {
                ...
                if (mRequired && mIndex == MAIN_INDEX) {
                    ...
                    mListener.onError(mIndex, R.string.error_value_too_large);
                }
            }
        }
        ...
        protected InitialResult doInBackground(Void... nothing) {
            try {
                ...
                try {
                    ...
                    // we can't fix recursion when call get_appr(), so I try catch it
                    return new InitialResult(R.string.timeout);
                }
    ...
    static int getLsdOffset(UnifiedReal val, String cache, int decIndex) {
        if (val == null || val.definitelyZero()) return Integer.MIN_VALUE;
    ...
    public boolean isMemory() {
        String memorySaved = mSharedPrefs.getString(KEY_PREF_MEMORY_SAVED, "");
        if (memorySaved != null && !memorySaved.equalsIgnoreCase("")) {
            return true;
        } else {
            return false;
        }
    ...
    public String getMemory() {
        return mSharedPrefs.getString(KEY_PREF_MEMORY_SAVED, "");
    }

    public void saveMemory(String memory) {
        String groupSeparator = String.valueOf(DecimalFormatSymbols.getInstance().getGroupingSeparator());
        if (".".equals(groupSeparator)) {
            groupSeparator = "\\.";
        }
        mSharedPrefs.edit()
                .putString(KEY_PREF_MEMORY_SAVED, memory.replaceAll(groupSeparator, ""))
                .apply();
    }

    public void addMemory(String add) {
        final String memory = mSharedPrefs.getString(KEY_PREF_MEMORY_SAVED, "");
        BigDecimal memoryNumber = null;
        try {
            memoryNumber = new BigDecimal(memory);
            BigDecimal addNumber = new BigDecimal(add.replaceAll(String.valueOf(DecimalFormatSymbols.getInstance(Locale.US).getGroupingSeparator()), ""));
            memoryNumber = memoryNumber.add(addNumber);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (memoryNumber != null) {
            mSharedPrefs.edit()
                    .putString(KEY_PREF_MEMORY_SAVED, memoryNumber.toString())
                    .apply();
        } else {
            mSharedPrefs.edit()
                    .putString(KEY_PREF_MEMORY_SAVED, "")
                    .apply();
        }
    }

    public void minusMemory(String minus) {
        final String memory = mSharedPrefs.getString(KEY_PREF_MEMORY_SAVED, "");
        BigDecimal memoryNumber = null;
        try {
            memoryNumber = new BigDecimal(memory);
            BigDecimal minusNumber = new BigDecimal(minus.replaceAll(String.valueOf(DecimalFormatSymbols.getInstance(Locale.US).getGroupingSeparator()), ""));
            memoryNumber = memoryNumber.subtract(minusNumber);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (memoryNumber != null) {
